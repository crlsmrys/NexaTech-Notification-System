/**
 * NexaTech Notification System - Server 2 (Secondary Server on Port 3001)
 * 
 * Features:
...
io.on('connection', (socket) => {
  ...
  socket.on('user_join', (userData) => {
    ...
    console.log(`✅ ${user.name} (${user.role}) joined from ${config.server2.name}`);
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (user) {
      ...
      console.log(`👋 ${user.name} disconnected from ${config.server2.name}`);
    }
  });
...
// Start server
const PORT = config.server2.port;
